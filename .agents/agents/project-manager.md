...
### 🏗️ Systems Architect
- [ ] Review DB schema changes (Eloquent migrations)
- [ ] Validate scalability of approach

### 🔧 Backend
- [ ] Migration: add `orders` table + Eloquent model
- [ ] `POST /api/v1/orders` controller + FormRequest + service + tests
- [ ] Laravel Job: send order confirmation email (Queue)

### 🎨 Frontend
- [ ] Order form component (Vue 3 + Inertia form)
- [ ] Order confirmation page (.vue Inertia page)
...
### ✅ QA
- [ ] Feature test: full checkout flow (Pest)
- [ ] E2E test: Playwright checkout
...
- [ ] Tests passing in CI (php artisan test --coverage)
- [ ] Deployed to staging
